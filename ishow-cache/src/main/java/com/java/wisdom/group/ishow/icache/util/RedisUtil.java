package com.java.wisdom.group.ishow.icache.util;
...
//	}
    ...
    // 过期时间1000 * 60 * 5
    private final static long expiration = 60 * 3;
    ...
    public String create(String key){
        ...
        String secretKey;
        if(hasKey(key)){
            if(getExpire(key) < lastTime){
                set(key, SecureUtil.md5(UUID.randomUUID().toString()),expiration);
            }
            secretKey = (String)get(key);
        }else{
            secretKey = SecureUtil.md5(UUID.randomUUID().toString());
            set(key, secretKey, expiration);
        }
        return secretKey;
    }
